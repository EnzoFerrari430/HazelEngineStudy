...
namespace Hazel {
    ...
    namespace Utils {
        ...
        static const char* GLShaderStageCacheOpenGLFileExtension(uint32_t stage)
        {
            switch (stage)
            {
            case GL_VERTEX_SHADER:      return ".cache_opengl.vert";
            case GL_FRAGMENT_SHADER:    return ".cache_opengl.frag";
            }
            HZ_CORE_ASSERT(false);
            return "";
        }
        ...
        static const char* GLShaderStageCacheVulkanFileExtension(uint32_t stage)
        {
            switch (stage)
            {
            case GL_VERTEX_SHADER:      return ".cache_vulkan.vert";
            case GL_FRAGMENT_SHADER:    return ".cache_vulkan.frag";
            }
            HZ_CORE_ASSERT(false);
            return "";
        }
        ...
        : m_FilePath(filepath)
    {
        ...
        auto lastSlash = filepath.find_last_of("/\\"); //支持斜杠与反斜杠
    ...
    std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
    {
        ...
        while (pos != std::string::npos)
        {
            ...
            shaderSources[Utils::ShaderTypeFromString(type)] = (pos == std::string::npos) ? source.substr(nextLinePos) : source.substr(nextLinePos, pos - nextLinePos); // Fix PreProcess Funtion Bugs
        }
    ...
    void OpenGLShader::CompileOrGetVulkanBinaries(const std::unordered_map<GLenum, std::string>& shaderSources)
    {
        ...
        for (auto&& [stage, source] : shaderSources)
        {
            ...
            std::filesystem::path cachePath = cacheDirectory / (shaderFilePath.filename().string() + Utils::GLShaderStageCacheVulkanFileExtension(stage));
            ...
            else
            {
                ...
                shaderData[stage] = std::vector<uint32_t>(module.cbegin(), module.end());
    ...
    void OpenGLShader::CompileOrGetOpenGLBinaries()
    {
        ...
        {
            std::filesystem::path shaderFilePath = m_FilePath;
            std::filesystem::path cachePath = cacheDirectory / (shaderFilePath.filename().string() + Utils::GLShaderStageCacheOpenGLFileExtension(stage));
            std::ifstream in(cachePath, std::ios::in | std::ios::binary);
            if (in.is_open())
            {
                in.seekg(0, std::ios::end);
